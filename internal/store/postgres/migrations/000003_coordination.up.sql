...
CREATE INDEX circuit_state_reconciliation_idx ON backend_circuit_state(state,opened_at) WHERE state IN ('open','half_open');
...
CREATE TABLE backend_circuit_active_failures (attempt_id UUID PRIMARY KEY, backend_id BIGINT NOT NULL, backend_revision BIGINT NOT NULL, event_at TIMESTAMPTZ NOT NULL);
CREATE INDEX circuit_active_failures_backend_idx ON backend_circuit_active_failures(backend_id,backend_revision,event_at);
...
CREATE TABLE coordination_replicas (replica_id UUID PRIMARY KEY, started_at TIMESTAMPTZ NOT NULL, heartbeat_at TIMESTAMPTZ NOT NULL, heartbeat_expires_at TIMESTAMPTZ NOT NULL, binary_version TEXT NOT NULL, coordination_contract_version INTEGER NOT NULL, policy_fingerprint BYTEA NOT NULL);
