...
func TestReleaseComposeUsesPublishedGatewayImageWithoutBuildingSource(t *testing.T) {
	...
	gateway := config.Services["gateway"]
	if gateway.Image != "ghcr.io/rislanov/vllm-priority-gateway:0.4.0" {
		t.Errorf("gateway image = %q, want published v0.4.0 image", gateway.Image)
	}
	if len(gateway.Build) != 0 && string(gateway.Build) != "null" {
...
func TestReleaseExampleSelectsCurrentPublishedVersion(t *testing.T) {
	...
	config := renderReleaseCompose(t, repositoryRoot, envPath)
	if image := config.Services["gateway"].Image; image != "ghcr.io/rislanov/vllm-priority-gateway:0.4.0" {
		t.Errorf("gateway image from .env.example = %q, want published v0.4.0 image", image)
	}
}
