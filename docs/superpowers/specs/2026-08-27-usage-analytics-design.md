...
The recorder owns a bounded channel and one SQLite writer goroutine. It groups pending events into transactions, flushing on a short interval or batch-size threshold. Once an authenticated request has a resolved public-model identity, it reserves one bounded recorder-completion slot before policy rejection or upstream forwarding. Queue saturation therefore applies cancellable backpressure before new inference work or an error response begins, rather than blocking handler return after response bytes have been written or silently dropping analytics rows. A successful reservation returns an opaque, process-local lifecycle handle; enriched metadata staging and terminal handoff must use that exact handle rather than request ID alone. This guarantees that terminal response completion can hand the staged row to the recorder without blocking HTTP response finalization and that a refused duplicate lifecycle cannot consume the original reservation. Aggregate observers reserve only peers that also accept terminal completion, roll earlier peers back in reverse order when a later reservation fails, and complete or roll back each successful peer exactly once. A live-context reservation refusal returns the retryable `503` / `gateway_unavailable` contract, while request-context cancellation preserves the disconnect/no-row path.
