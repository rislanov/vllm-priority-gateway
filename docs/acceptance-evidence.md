...
The default suite still runs a smaller performance smoke test, but only the opt-in run enforces timing because shared CI and developer machines can be noisy. The SQLite performance harness uses the same bounded asynchronous API-key activity recorder as the application; the CRUD harness retains synchronous writes for immediate storage assertions. Gateway-added numbers are differences of gateway/direct quantiles, not quantiles of paired per-request overhead. SQLite parallelism is 16; the PostgreSQL reference test uses parallelism 1, so these results do not directly compare database throughput.
...
### Renewal, cache, and recovery review cycle — 2026-09-15

The implementation under test is `09259873290a7db266aff7a8d5d2ca423cb26931`, including the earlier admission-contention and E2E-fixture fixes in `ca32cf7` and `53d3562`. The final recovery replay-race change was tested separately on Windows and included in the complete Linux race run. This evidence supersedes the earlier GPU and SQLite reference-performance gaps for the specific development topology below; it does not certify an unspecified production topology.

Regressions first reproduced per-lease renewal timeout, loss of committed renewal results after a later batch error, corruption of a live receipt by an incorrect renewal identity, analytics writer backpressure without a deadline, older concurrent circuit snapshot publication, serial circuit reconciliation timeout, and recovery becoming ready while a new failure awaited replay. Fixes use bounded bulk renewal, partial-result consumption, five-second analytics writes, serialized cache publication, bulk expiry reconciliation, and a recovery failure epoch invalidated before buffering. Independent reviewers checked renewal/analytics, circuit expiry/locking/publication, and admission/authentication/deletion/rate-accounting integration.

Transport tests cut the PostgreSQL `COMMIT` response before forwarding it and verify that admission and half-open acquisition retry the same UUID without duplicating leases, permits, or RPM debits. These tests passed with the existing pgx 5.11 retry classification: the earlier static suspicion about all lost-COMMIT responses was not confirmed, so no speculative retry change was made.

Validation:

- Windows amd64, Go 1.27.0: `go test -count=1 -p 1 -timeout 15m ./...` passed with PostgreSQL 16.15, PostgreSQL 15.19 refusal, and PgBouncer 1.24.0 transaction pooling (`max_prepared_statements=0`). The PostgreSQL package took 38.562 s. After the final replay-race fix, `go test -count=1 ./internal/monitor ./internal/coordination ./cmd/gateway` also passed.
- Linux amd64, CGO enabled, `golang:1.27-bookworm`: `go test -count=1 -p 1 -race -timeout 15m ./...` passed with the same database/pooler gates over a dedicated Docker bridge network. The PostgreSQL package took 37.331 s. No data race was reported. An earlier run routed through `host.docker.internal` failed two test-fixture startup connections at their ten-second deadline; it is not counted as passing. The complete bridge-network rerun passed without extending deadlines or changing assertions.
- `go vet ./...`, `go mod verify`, `go build ./cmd/...`, Windows gateway/E2E builds, static Linux amd64 gateway/fake-vLLM/loadgen/E2E builds, and the static Linux arm64 gateway build passed. All 176 Go source files were gofmt-canonical after normalizing checkout CRLF; `git diff --check` passed.
- Docker image build passed. SQLite and PostgreSQL containers returned 200 for `/healthz`, `/readyz`, and `/coordination-readyz`; both exited 0 on shutdown.
- The PostgreSQL test target now runs the complete internal PostgreSQL coordination package, including the new concurrent-refresh database regression, before gateway and external PostgreSQL tests.

Real GPU validation used NVIDIA RTX 4070 Ti, driver 616.56, two `vllm/vllm-openai:v0.28.0` containers serving `Qwen/Qwen3-0.6B`, priority scheduling, one sequence per backend, maximum model length 1024, and eager execution. Checked-in smoke, priority, and resilience E2E tests passed for both SQLite and PostgreSQL. Each of `/v1/completions`, `/v1/chat/completions`, and `/v1/responses` passed in streaming and nonstreaming mode. Cancellation incremented disconnect telemetry and drained gateway/upstream work. Restart preserved configuration in both profiles. Two PostgreSQL gateway processes verified shared concurrency while a stream crossed a renewal interval, cancellation release, shared RPM results `200, 200, 429`, and key revocation propagation. The full Python orchestration completed successfully with all 27 recorded checks passing; it invoked the compiled checked-in E2E tests without replacing their assertions.

After stopping the GPU containers, strict reference-performance tests used an Intel i5-14600KF Windows host, Docker Desktop Linux amd64, Go 1.27.0, PostgreSQL 16.15, and a test process sharing the database loopback namespace. SQLite completed 500 requests at parallelism 16 with added p50 **1.107 ms**, p99 **1.740 ms**. PostgreSQL completed 500 requests at parallelism 1 per run. The first result narrowly exceeded the p50 gate (**5.051/6.720 ms** p50/p99); three subsequent quiet-host runs all passed the unchanged `<5/<20 ms` gate: **4.866/13.536**, **4.896/5.761**, and **4.932/6.911 ms**. This demonstrates limited p50 headroom and environment sensitivity, not a portable latency guarantee. No failed measurement was discarded or threshold relaxed.

Local raw output is retained under `dist/review-cycle-20260915/` and is gitignored. This checked-in summary is the durable evidence. The HA-endpoint opt-in test was skipped because no actual HA endpoint was supplied. Actual synchronous promotion, loss of synchronous durability, and former-primary fencing remain the explicit deployment acceptance gates in section 19.2; a standalone database or reopening its connection does not establish them. Production-model capacity and latency calibration also remain deployment-specific.

### Admission, recovery, and latency remediation — 2026-09-15

The working tree based on `ceaae6e483b438ff29fd6627ea6798bb37e106bb` changes the default PostgreSQL coordination deadline from 50 to 500 ms and batches ordered admission statements to reduce network waits under the shared pool lock. Lock acquisition and subsequent counts remain separate statements for fresh READ COMMITTED snapshots; synchronous commit and durable replay/concurrency enforcement remain enabled. An explicit 50 ms configuration still uses that deadline. A warmed 16-request burst through two coordinators admitted only 6/16 before the change, 8/16 with batching alone at 50 ms, and passed three times with the new default, including durable lease counts and rejection of request 17.

Real vLLM 0.28.0/Qwen3-0.6B priority tests on the RTX 4070 Ti passed three isolated runs with a fresh gateway process each time. A preceding series in one process passed twice and then failed to saturate both backends; that failure is retained and its cause is unresolved. Shared concurrency, renewal after 32 seconds, cancellation release, shared RPM and key revocation passed across two real gateway processes sharing PostgreSQL. The original resilience test passed ten isolated repeats; after adding a bounded wait for the independently published pool projection, the updated test also passed ten repeats. A separate expired-probe fixture now controls database expiry/cooldown explicitly rather than depending on a 15 ms permit and 1 ms cooldown; five repeats and the full suite passed.

Three repeats of each 500-request latency gate kept the strict added p50 < 5 ms and p99 < 20 ms budgets. PostgreSQL harness median added p50 improved from 9.181 to 7.116 ms on Windows and from 4.973 to 4.310 ms on Linux sharing database loopback; Windows passed 0/3 and Linux 3/3. The SQLite performance harness now uses the production asynchronous, coalescing API-key activity recorder instead of synchronous per-request CRUD writes. That method correction passed 3/3 on Windows and 2/3 on Linux; one Linux run had added p99 50.604 ms, with cause unresolved. This SQLite before/after comparison does not demonstrate a production storage optimization.

An additional test of the actual scratch production image (UID/GID 65532, SQLite named data volume, separate fake backend, shared loopback with PostgreSQL in Docker Desktop/WSL2) passed 3/3 for both profiles. Added p50/p99 ranges were 0.550–0.679 / 0–7.838 ms for SQLite and 4.238–4.323 / 5.061–5.654 ms for PostgreSQL. The separate Windows application passed 3/3 for SQLite and 0/3 for PostgreSQL (added p50 7.743–10.347 ms, p99 26.513–49.617 ms). Added latency is a difference of independently measured quantiles, with negative differences clamped to zero; SQLite uses parallelism 16 and PostgreSQL 1. These measurements qualify the stated local topology, not cross-host production latency or equivalent throughput.

Full Windows tests with real PostgreSQL passed; the full Linux race run passed, followed by a race rerun of packages changed by the shared-recorder extraction. Vet, command builds, Docker production build, Linux amd64/arm64 static builds and PgBouncer 1.24 transaction-mode runtime/reassignment tests passed. Detailed Russian report, raw successes and failures, profiles, JSON and temporary reproduction scripts are in `dist/remediation-20260915/REPORT.md`. Windows/PostgreSQL latency, the isolated Linux/SQLite tail and the same-process priority saturation failure remain open; no universal latency or production HA acceptance is claimed.

The subsequent pre-commit verification repeated `go test -count=1 -p 1 -timeout 15m ./...` against the same isolated PostgreSQL 16 database. It failed only in `TestPostgresClosedCircuitFailureCountExpiresWithWindow`: replay after rolling prune returned a closed circuit with FailureCount=0 instead of 1. Five isolated repeats of that test then passed. The fixture uses a 40 ms failure window; the failure's cause is not confirmed and has not been fixed. Fresh vet, command builds and diff checks passed. Earlier passing suites remain historical evidence; the latest full suite is not green.

### Circuit window fixture correction — 2026-09-15

The pre-commit failure above was reproduced by inserting an 80 ms delay before replay. Replay returns the current failure-window count; the newer event can legitimately expire during the fixture's SQL checks under the former 40 ms window, so expecting 1 unconditionally was incorrect. The corrected fixture uses a one-minute window and explicitly ages the expiring event projections in the test database while preserving immutable receipt timestamps. It verifies replay with one current failure and again after all failures expire, including absence of resurrection of the original pruned event. Production circuit logic is unchanged.

The corrected test passed 20/20 Windows repeats and 20/20 Linux race repeats against PostgreSQL 16. The full Windows suite with real PostgreSQL (`go test -count=1 -p 1 -timeout 15m ./...`) then passed, along with fresh vet and diff checks. Logs are in `dist/remediation-20260915/windows-circuit-fixed-full.log` and `dist/merge-readiness-20260915/linux-circuit-window-race.log`. This supersedes the latest-full-suite failure status in the preceding historical record; the separately recorded latency and priority saturation limitations remain open.

### Priority burst preparation correction — 2026-09-15

The priority saturation symptom was reproduced with a diagnostic overlay using a 256-token baseline to ensure it was sampled by metrics. The baseline backend retained EWMA pressure around 0.079; the other backend had zero pressure and received all 16 burst requests. The pool correctly remained normal because its pressure is the minimum across available backends. Before the correction, the test assumed that a completed baseline left equal idle routing conditions for a finite burst.

The checked-in priority test now waits after its original four-token baseline for at least the expected number of eligible backends, zero upstream running/waiting, zero gateway inflight/waiting, and per-backend pressure at most 0.001. This uses the existing saturation timeout and a context-bound Admin request; saturation and priority assertions are unchanged. Unit tests cover a normal pool with residual pressure, queued/inflight work and stale metrics, plus a stalled endpoint. The diagnostic baseline then passed with an initial 8/8 burst distribution and both backends saturated. Production routing and circuit policies are unchanged. Diagnostic failures, state snapshots and the successful overlay run are retained in `dist/merge-readiness-20260915`.

The corrected checked-in priority test passed 5/5 real GPU repeats in one gateway process without restarts. The final full Windows suite and final full Linux race suite, both against real PostgreSQL 16, passed after both fixture corrections; fresh vet, command builds, gofmt and diff checks passed. See `dist/merge-readiness-20260915/REPORT.md`, `same-process-fixed-*-priority.log`, `windows-final-full.log` and `linux-final-race.log`. The first Linux attempt encountered temporary overlay sources as a separate Go package; those sources were retained as `.go.txt` artifacts and the full command reran successfully. The reproduced circuit-window and priority-preparation failures above are corrected; the previously recorded Windows/PostgreSQL latency and isolated Linux/SQLite harness p99 remain open.
